...
def main():
    ...
    wandb.init(project="Imagenet Mixup 100epoch", entity="neonsign", name = 'mixup159.109_' + configs.output_name)
...
#         #
        ...
        images_mixup = input[~ignore_mask]
        images_ignore = input[ignore_mask]

        rand_index = torch.randperm(images_mixup.size()[0]).cuda()
        target_a = torch.cat((target_mixup, target_ignore), 0)
        target_b = torch.cat((target_mixup[rand_index], target_ignore), 0)
        input = torch.cat((images_mixup, images_ignore), 0) * lam + torch.cat((images_mixup[rand_index], images_ignore), 0) * (1. - lam)
